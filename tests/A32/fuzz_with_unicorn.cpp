...
static bool ShouldTestInst(u32 instruction, u32 pc, bool is_last_inst) {
    const A32::LocationDescriptor location{pc, {}, {}};
    IR::Block block{location};
    bool should_continue = A32::TranslateSingleInstruction(block, location, instruction);
...
static u32 GenRandomInst(u32 pc, bool is_last_inst) {
...
static Dynarmic::A32::UserConfig GetUserConfig(ArmTestEnv& jit_env) {
    Dynarmic::A32::UserConfig jit_user_config;
    jit_user_config.callbacks = &jit_env;
    return jit_user_config;
}

static void RunTestInstance(Dynarmic::A32::Jit& jit, A32Unicorn& uni, ArmTestEnv& jit_env, ArmTestEnv& uni_env,
                            const A32Unicorn::RegisterArray& regs, const A32Unicorn::ExtRegArray& vecs,
                            const std::vector<u32>& instructions, const u32 cpsr, const u32 fpscr) {
    const u32 initial_pc = regs[15];

    std::copy(instructions.begin(), instructions.end(), jit_env.code_mem.begin() + initial_pc / 4);
    std::copy(instructions.begin(), instructions.end(), uni_env.code_mem.begin() + initial_pc / 4);
    jit_env.code_mem[initial_pc / 4 + instructions.size()] = 0xEAFFFFFE; // B .
    uni_env.code_mem[initial_pc / 4 + instructions.size()] = 0xEAFFFFFE; // B .
    ...
    SCOPE_FAIL {
        ...
        for (size_t i = 0; i < regs.size(); ++i)
            fmt::print("{:3s}: {:08x} {:08x} {}\n", static_cast<A32::Reg>(i), uni.GetRegisters()[i], jit.Regs()[i], uni.GetRegisters()[i] != jit.Regs()[i] ? "*" : "");
        for (size_t i = 0; i < vecs.size(); ++i)
            fmt::print("s{:2d}: {:08x} {:08x} {}\n", static_cast<size_t>(i), uni.GetExtRegs()[i], jit.ExtRegs()[i], uni.GetExtRegs()[i] != jit.ExtRegs()[i] ? "*" : "");
    ...
    {
        auto regs = uni.GetRegisters();
        const bool is_thumb = (jit.Cpsr() & (1 << 5)) != 0;
        regs[15] &= (is_thumb ? 0xFFFFFFFE : 0xFFFFFFFC);
        uni.SetRegisters(regs);
    }
...
TEST_CASE("A32: Single random instruction", "[arm]") {
    ...
    Dynarmic::A32::UserConfig jit_user_config;
    jit_user_config.callbacks = &jit_env;
    Dynarmic::A32::Jit jit{jit_user_config};
    A32Unicorn uni{uni_env};

    A32Unicorn::RegisterArray regs;
    A32Unicorn::ExtRegArray ext_reg;
